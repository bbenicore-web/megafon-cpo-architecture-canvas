...
3. **Удаление блоков** — кнопка «Удалить» в панели (плитки, секции, CPO, команды, шаги flow, зоны ролей, RACI, интеграции, метрики). У секций доменов также есть кнопка «× Секцию».
4. Используйте кнопки **«+ Добавить элемент»** в секциях доменов, интеграции и RACI.
5. **Сохранить черновик** — данные сохраняются в localStorage браузера.
6. **Экспорт JSON** или **Экспорт data.js** — скачайте файл и замените `data.js` в репозитории, чтобы опубликовать изменения.
